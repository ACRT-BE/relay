...
export default function ScheduleEditor({ race }: Props) {
  ...
  const loadAll = async () => {
    ...
    const [{ data: d1 }, { data: d2 }] = await Promise.all([
      ...
      race ? supabase.from('stints').select('*').eq('race_id', race.id).order('order_index') : Promise.resolve({ data: [] as Stint[] } as any),
    ]);
    setDrivers(d1 ?? []);
    setStints((d2 as Stint[]) ?? []);
    setLoading(false);
  };

  useEffect(() => { loadAll(); /* eslint-disable-next-line */ }, [race?.id]);

  // Realtime
  useEffect(() => {
    const chDrivers = supabase.channel('drivers-ch').on('postgres_changes', { event: '*', schema: 'public', table: 'drivers' }, loadAll).subscribe();
    const chStints = supabase.channel('stints-ch').on('postgres_changes', { event: '*', schema: 'public', table: 'stints' }, loadAll).subscribe();
    return () => {
  ...
  const addStint = async () => {
    ...
    const { data, error } = await supabase.from('stints').insert({ race_id: race.id, driver_id: null, order_index, duration_minutes: 10 }).select().single();
    if (!error && data) setStints((prev) => [...prev, data as Stint]);
  };
  ...
  const removeStint = async (stintId: string) => {
    ...
    setStints(prev => prev.filter(s => s.id !== stintId).map((s, i) => ({ ...s, order_index: i })));
    await supabase.from('stints').upsert(stints.filter(s => s.id !== stintId).map((s, i) => ({ id: s.id, order_index: i })));
  };

  const swapDrivers = async (a: number, b: number) => {
    const sA = stints[a]; const sB = stints[b];
    if (!sA || !sB) return;
    const copy = [...stints];
    copy[a] = { ...sA, driver_id: sB.driver_id };
    copy[b] = { ...sB, driver_id: sA.driver_id };
    setStints(copy);
    await supabase.from('stints').upsert([
    ...
    <div className="space-y-3">
      <div className="card flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-slate-400">{race.type} • départ {formatHM(race.start_time)}</div>
        </div>
        <button onClick={addStint} className="btn bg-brand-500">+ Relais</button>
      </div>

      <div className="card space-y-3">
        {computed.length === 0 && <div className="text-sm text-slate-400">Aucun relais pour l’instant.</div>}
        {computed.map((s, i) => {
          ...
          return (
            <div key={s.id} className="flex items-center gap-2 bg-white/5 rounded-xl p-2">
              ...
              <div className={clsx("h-8 w-1 rounded", d?.color ? "")} style={{ backgroundColor: d?.color || 'transparent' }} />
              <div className="flex-1">
                <div className="text-sm font-medium">{d ? d.name : <span className="text-slate-400">Non assigné</span>}</div>
                <div className="text-xs text-slate-400">{formatHM(s.start_time)} • {s.duration_minutes} min</div>
              </div>
              <select className="input text-sm" value={s.driver_id ?? ''} onChange={(e) => assignDriver(s.id, e.target.value || null)}>
                <option value="">— Pilote —</option>
                {drivers.map(dr => <option key={dr.id} value={dr.id}>{dr.name}</option>)}
              </select>
              <input type="number" className="input w-20 text-sm" value={s.duration_minutes} min={1} onChange={(e) => changeDuration(s.id, parseInt(e.target.value || '0', 10))} />
              <div className="flex flex-col gap-1">
                <button className="btn bg-white/10" onClick={() => moveStint(i, -1)}>↑</button>
                <button className="btn bg-white/10" onClick={() => moveStint(i, 1)}>↓</button>
              </div>
...
function SwapWidget({ maxIndex, onSwap }: { maxIndex: number; onSwap: (a:number,b:number)=>void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <input className="input w-16" type="number" min={1} max={maxIndex} value={a} onChange={(e)=>setA(parseInt(e.target.value||'1',10))} />
      <span className="opacity-70">↔</span>
      <input className="input w-16" type="number" min={1} max={maxIndex} value={b} onChange={(e)=>setB(parseInt(e.target.value||'2',10))} />
      <button className="btn bg-white/10" onClick={()=>onSwap(a,b)}>Permuter</button>
    </div>
  );
}
...
function DriverManager({ drivers, onChange }: { drivers: Driver[]; onChange: (d:Driver[])=>void }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <input className="input flex-1" placeholder="Nom du pilote" value={name} onChange={(e)=>setName(e.target.value)} />
        <input type="color" className="h-10 w-14 rounded-xl border border-white/20" value={color} onChange={(e)=>setColor(e.target.value)} />
        <button className="btn bg-brand-500" onClick={create}>+ Pilote</button>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {drivers.map(d => (
          <div key={d.id} className="rounded-xl bg-white/5 p-2 flex items-center justify-between">
            ...
            <button className="btn bg-red-600" onClick={()=>remove(d.id)}>Suppr</button>
          </div>
        ))}
      </div>
    </div>
